...
let expName = 'test_posner_A';  // from the Builder filename that created this script
...
function feedbackRoutineBegin(trials) {
  return function () {
    ...
    feed_text.setColor(new util.Color(color_feed));
    feed_text.setText(msg);
    // keep track of which components have finished
    feedbackComponents = [];
    feedbackComponents.push(feed_text);
...
function feedbackRoutineEachFrame(trials) {
  return function () {
    ...
    // *feed_text* updates
    if (t >= 0.0 && feed_text.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      feed_text.tStart = t;  // (not accounting for frame time here)
    ...
    if (feed_text.status === PsychoJS.Status.STARTED && t >= frameRemains) {
